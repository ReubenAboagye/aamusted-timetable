...
-- Appended schema (from schema.sql)
-- =========================================
-- 1) DEPARTMENTS
...
CREATE TABLE lecturers (
  ...
  max_sessions_per_week INT DEFAULT 2,
  preferred_days JSON,                -- e.g. ["monday","wednesday"]
  unavailable_times JSON,             -- free-form if you need it in app logic
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (department_id) REFERENCES departments(id) ON DELETE CASCADE
...
-- =========================================
-- 10) LECTURER_COURSES (who is allowed to teach what)
-- =========================================
...
-- =========================================
-- 11) AVAILABILITY GUARDS (hard validation for sessions)
...
--     - Uses DAY + TIME_SLOT (slots are reused across days)
--     - Enforces no double-booking for class, room, lecturer
--     - Validates lecturer/session and course/session via composite FKs
-- =========================================
CREATE TABLE timetable (
  id INT PRIMARY KEY AUTO_INCREMENT,
  class_id INT NOT NULL,
  course_id INT NOT NULL,
  lecturer_id INT NOT NULL,                 -- main lecturer; co-teachers below
  room_id INT NOT NULL,
  time_slot_id INT NOT NULL,
  day ENUM('monday','tuesday','wednesday','thursday','friday','saturday','sunday') NOT NULL,
  semester_id INT NOT NULL,
  session_id INT NOT NULL,                  -- FK to sessions (multi-session stream)
  activity_type ENUM('lecture','tutorial','laboratory','seminar') DEFAULT 'lecture',
  is_confirmed BOOLEAN DEFAULT FALSE,
  notes TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

  -- Conflict prevention (class/room/lecturer cannot collide in same day+slot+semester+session)
  UNIQUE KEY uq_class_time    (class_id,    day, time_slot_id, semester_id, session_id),
  UNIQUE KEY uq_room_time     (room_id,     day, time_slot_id, semester_id, session_id),
  UNIQUE KEY uq_lecturer_time (lecturer_id, day, time_slot_id, semester_id, session_id),
  ...
  -- Valid only if course is offered in this session:
  CONSTRAINT fk_tt_course_session
    FOREIGN KEY (course_id, session_id)
    REFERENCES course_session_availability (course_id, session_id),

  -- Valid only if lecturer is available in this session:
  CONSTRAINT fk_tt_lecturer_session
    FOREIGN KEY (lecturer_id, session_id)
    REFERENCES lecturer_session_availability (lecturer_id, session_id),

  -- Valid only if class is tied to this session:
  CONSTRAINT fk_tt_class_session
    FOREIGN KEY (class_id, session_id)
    REFERENCES classes (id, session_id),

  -- Only schedule courses the class actually takes this semester:
  CONSTRAINT fk_tt_class_course_sem
    FOREIGN KEY (class_id, course_id, semester_id)
    REFERENCES class_courses (class_id, course_id, semester_id)
);

-- =========================================
-- 13) TIMETABLE_LECTURERS (team-teaching)
-- =========================================
...
-- =========================================
-- 2) SESSIONS  (multi-session streams)
--    e.g. Regular, Evening, Weekend, Sandwich, Distance
-- =========================================
-- =========================================
-- 2) PROGRAMS
--    Programs belong to departments (e.g. BSc Computer Science)
